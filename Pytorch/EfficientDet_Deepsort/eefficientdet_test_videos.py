...
class MOT(object):
    def __init__(self,
                 ...
                 threshold=0.4,     # Seuil de confiance lors la detection des objets
                 ...
                 max_dist=0.14,   # distance spatiale maximale pour associer les objets entre 2 frames successives   # SELON LES VITESSES PREVUES DES OBJETS 
                 max_iou_distance=0.7, # distance max pour associer 2 objets en termes d'iou
                 min_confidence=0.3,
                 nms_max_overlap=0.5,
                 
                 max_age=50,     #  nb maximal de frames pendant d'inactivité avant de supprimer une trajectoire
                 n_init=1,      # nb minimal de détections avant d'initialiser une nouvelle trajectoire, il faut qu'elle soit associee n_init fois consecutives avant de creer sa trajectoire
        ...
        self.video_output = os.path.join(dirname, f"MOT_VIDEOS/DeepSort_onMOT/{self.seq}_DS.mp4")
        # text path
        self.text_output = os.path.join(dirname, f"MOT_VIDEOS/DeepSort_onMOT/{self.seq}.csv")

        #self.video_src = f"/home/jovyan/Desktop/MOT_VIDEOS/{self.seq}.mp4"
        #self.video_output = f"/home/jovyan/Desktop/MOT_VIDEOS/DeepSort_onMOT/{self.seq}_DS.mp4"
        # text path
        #self.text_output =  f"/home/jovyan/Desktop/MOT_VIDEOS/DeepSort_onMOT/{self.seq}.csv"
        ...
        model.load_state_dict(
            torch.load(os.path.join(dirname, 'Pytorch/logs/mot/3/mot_pytorch/efficientdet-d3_36_39000.pth')))
        #model.load_state_dict(torch.load('/home/jovyan/Desktop/Pytorch/logs/mot/3/mot_pytorch/efficientdet-d3_36_39000.pth'))
    ...
    def _display(self, preds, imgs, text_recorder=None, track_result=None):  # afficher les resultats du tracking dans la video
        ...
        #mot17_file = open(f"/home/jovyan/Desktop/compare_eff_rfcn/{self.seq}-FRCNN.txt", "a")
        mot17_file = open(os.path.join(dirname, f"compare_eff_rfcn/{self.seq}-FRCNN.txt"),"a")
    ...
    def detect_video(self):
        ...
        while True:
            ...
            # frame preprocessing
            ori_imgs, framed_imgs, framed_metas = preprocess_video(frame, max_size=self.input_size)  #pretrainer + rescale
