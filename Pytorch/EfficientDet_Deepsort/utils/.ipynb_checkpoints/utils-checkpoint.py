...
def preprocess_video_residual(*frame_from_video, max_size=512, mean=(0,0,0), std=(1,1,1)):
    ori_imgs = frame_from_video
    normalized_imgs = [(img / 255 - mean) / std for img in ori_imgs]
    imgs_meta = [aspectaware_resize_padding(img[..., ::-1], max_size, max_size,
                                            means=None) for img in normalized_imgs]
...
def tlwh_to_xyxy(boxes_tlwh):
    if isinstance(boxes_tlwh, torch.Tensor):
        boxes_xyxy = boxes_tlwh.clone()
    elif isinstance(boxes_tlwh, np.ndarray):
        boxes_xyxy = boxes_tlwh.copy()
    else:
        raise TypeError

    # Conversion
    boxes_xyxy[:, 0] = boxes_tlwh[:, 0]  # x1 = l
    boxes_xyxy[:, 1] = boxes_tlwh[:, 1]  # y1 = t
    boxes_xyxy[:, 2] = boxes_tlwh[:, 0] + boxes_tlwh[:, 2]  # x2 = l + w
    boxes_xyxy[:, 3] = boxes_tlwh[:, 1] + boxes_tlwh[:, 3]  # y2 = t + h
