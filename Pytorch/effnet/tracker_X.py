...
from Pytorch.EfficientDet_Deepsort.config import REID_CKPT, selected_target
from Pytorch.EfficientDet_Deepsort.deep_sort import build_tracker
from Pytorch.EfficientDet_Deepsort.efficientdet.utils import ClipBoxes, BBoxTransform
from Pytorch.Yolo.utils.utils import preprocess_video_frame
from Pytorch.effnet.utils.utils import postprocess, invert_affine
...
class Tracker:
    def __init__(self, base_net_model,resd_model, tracking_model, appearance_model, classes=None, args=None, cfg=None, compound_coef=2, compound_coef_resd=3):
        ...
        print(f"crop sizea{args.feature_crop_size}")
        ...
        self.tracking_thr = {'EFFICIENTDET': 0.5, 'DEEPSORT': 0.97,'PRIVATE': 0.95, 'DPM': 0.1, 'FRCNN': 0.8, 'SDP': 0.7, 'POI': 0.33}
...
#        self.im_roi_align = RoIAlign(crop_width=self.im_crop_size[2], crop_height=self.im_crop_size[1],
 #                                    transform_fpcoor=True)
        print(f"fearture crop size {self.feature_crop_size[2]} {self.feature_crop_size[1]}")
        ...
        self.roi_align_box_index = self.roi_align_box_index
        ...
        self.num_boxes = self.num_boxes
        self.track_feature = self.track_feature
        self.detection_feature = self.detection_feature

        self.image_size = torch.FloatTensor([0, 0])
 ...
 #       print('boxes scaled',boxes)
        """if in_data_type == 'feature':
            ...
            roi_align = RoIAlign(4, 4)
            croped_data = roi_align(image, boxes, box_index)
        print('croped_data', croped_data.shape)  # devrait imprimer torch.Size([1, 560, 9, 9])
     #   print('croped_data TEST',croped_data)"""

        if in_data_type == 'feature':
            print(f"in data {in_data.size()}")
            croped_data = self.feat_roi_align(in_data, boxes, box_index)
            print(f"croped {croped_data.size()}")
    ...
    def reset(self, tracking_output_file=None, detection_output_file=None):    # save les résultats de détection et tracking et faire un reset pour la prochaine vidéo
       ...
       # print('self.tracking_results',self.tracking_results)
        ...
        if tracking_output_file is not None:
            ...
            print(self.tracking_results)

            tracking_results = self.tracking_results.cpu().numpy()
        ...
        if detection_output_file is not None and self.detection_results is not None:
            ...
            detection_results = self.detection_results.cpu().numpy()
...
#        print('im_data_tmp',im_data_tmp.shape)
       # print('im_scale_tmp',im_scale_tmp)
        ...
        self.image_size = torch.FloatTensor([resized_width, resized_height])
